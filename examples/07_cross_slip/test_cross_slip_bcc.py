...
def init_cross_slip_config_bcc(Lbox):
    cell = pyexadis.Cell(h=Lbox*np.eye(3), is_periodic=[1,1,1])
    nodes, segs = [], []
    
    theta = 0.0 # screw
    burg = 1.0/np.sqrt(3.0)*np.array([1.,1.,1.])
...
def test_cross_slip_bcc():
    
    pyexadis.initialize()
    
    Lbox = 1000.0
    N, Rorient = init_cross_slip_config_bcc(Lbox)
    ...
    mobility   = MobilityLaw(mobility_law='BCC_0B', state=state, Medge=2000.0, Mscrew=20.0, Mclimb=1.0, vmax=4000.0)

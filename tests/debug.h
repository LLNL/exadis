...
struct Debug {
     ...
     *---------------------------------------------------------------------*/
    int verify_Burgers(SerialDisNet* net)
    {
     ...
     *---------------------------------------------------------------------*/
    void write_force(System* system, std::string file) {
     ...
     *---------------------------------------------------------------------*/
    void write_velocity(System* system, std::string file) {
    ...
    void print_node(N* net, int i, int print_nei=0)
    {
        ...
        for(int j = 0; j < conn[i].num; j++) {
            ...
            printf("  conn[%d]: node = %d, seg = %d, order = %d\n",j,k,s,o);
            printf("            burg = %e %e %e\n",b.x,b.y,b.z);
        }
        if (print_nei) {
            for(int j = 0; j < conn[i].num; j++) {
     ...
     *---------------------------------------------------------------------*/
    void extract_nodes(SerialDisNet* net, std::vector<int> nodes, std::string filename)
    {
     ...
     *---------------------------------------------------------------------*/
    void extract_segs(SerialDisNet* net, std::vector<int> segs, std::string filename)
    {
        SerialDisNet* ncopy = new SerialDisNet(net->cell);
        ncopy->nodes = net->nodes;
        ncopy->segs = net->segs;
